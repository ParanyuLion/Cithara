...
function AudioPlayer({ song }) {
  ...
  function drawStatic() {
    ...
    for (let i = 0; i < N; i++) {
      const h = Math.max(4, (0.08 + 0.25 * Math.abs(Math.sin(i * 0.38)) * Math.sin(i * 0.13 + 1)) * H);
  ...
  function stopVisualizer() {
    if (animFrmRef.current) { cancelAnimationFrame(animFrmRef.current); animFrmRef.current = null; }
    drawStatic();
  }

  React.useEffect(() => {
    drawStatic();
    return () => { if (animFrmRef.current) cancelAnimationFrame(animFrmRef.current); };
  }, []);
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', gap: '24px', alignItems: 'flex-start' }}>
        ...
        <div style={{
          ...
          fontSize: '56px', color: 'rgba(255,255,255,0.75)',
        ...
        <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          {/* Song info */}
          <div>
            <div style={{ fontSize: '18px', fontWeight: 800, color: 'var(--text)', marginBottom: '3px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
              {song.title}
            </div>
          ...
          <canvas
            ...
            width={800}
            height={64}
            style={{ width: '100%', height: '64px', borderRadius: '6px', display: 'block' }}
          />

          {/* Seek bar with timestamps on both ends */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <span style={{ color: 'var(--text-muted)', fontSize: '11px', fontFamily: 'var(--font-mono)', flexShrink: 0, width: '34px' }}>
              {fmt(current)}
            </span>
            <div
              ref={seekRef}
              onClick={handleSeek}
              style={{ flex: 1, height: '4px', background: 'var(--surface-3)', borderRadius: '2px', cursor: 'pointer', position: 'relative' }}
              onMouseEnter={e => { e.currentTarget.style.height = '6px'; }}
              onMouseLeave={e => { e.currentTarget.style.height = '4px'; }}
            >
              <div style={{ height: '100%', width: `${progress}%`, background: 'var(--accent)', borderRadius: '2px', pointerEvents: 'none', transition: 'width 0.1s' }} />
            </div>
            <span style={{ color: 'var(--text-muted)', fontSize: '11px', fontFamily: 'var(--font-mono)', flexShrink: 0, width: '34px', textAlign: 'right' }}>
              {fmt(duration)}
            </span>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flex: 1, minWidth: '140px' }}>
              <span style={{ color: 'var(--text-muted)', fontSize: '11px', fontWeight: 600, flexShrink: 0 }}>Vol</span>
              <input
  ...
  return (
    <div style={{ marginBottom: '28px' }}>
      ...
      <div style={{
        ...
        <button
          onClick={handleCopy}
          onMouseEnter={e => { if (!copied) { e.currentTarget.style.background = 'rgba(29,185,84,0.15)'; e.currentTarget.style.color = 'var(--accent)'; }}}
